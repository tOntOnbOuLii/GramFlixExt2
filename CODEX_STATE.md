...
- Release GramFlix v1.4.19 assemblee (plugin version 24, artefacts 96 812 octets copies vers `gramflix-all.cs3` et `gramflix-1jour1film.cs3`).

## Tests / verifications
- `./gradlew.bat :app:clean :app:make` OK -> artefact `app/build/app.cs3` recopie vers `gramflix-all.cs3` + `gramflix-1jour1film.cs3` (96 812 o, plugin 24).
...
- Verifier sur GitHub que la release v1.4.19 et les assets `.cs3` restent accessibles (retenter upload si le token expire).
