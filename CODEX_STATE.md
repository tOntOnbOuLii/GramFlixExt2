...
- Accueil Coflix : les sections populaires sont maintenant construites via TMDB pour afficher de vrais films/séries plutôt que des genres marketing.
- Release GramFlix v1.4.18 assemblee (plugin version 23, artefacts 96 417 octets copies vers `gramflix-all.cs3` et `gramflix-1jour1film.cs3`).

## Tests / verifications
- `./gradlew.bat :app:clean :app:make` OK -> artefact `app/build/app.cs3` recopie vers `gramflix-all.cs3` + `gramflix-1jour1film.cs3` (96 417 o, plugin 23).
...
- Verifier sur GitHub que la release v1.4.18 et les assets `.cs3` restent accessibles (retenter upload si le token expire).
