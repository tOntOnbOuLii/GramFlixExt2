...
- Release GramFlix v1.4.16 assemblee (plugin version 21, artefacts 95 654 octets copies vers `gramflix-all.cs3` et `gramflix-1jour1film.cs3`).

## Tests / verifications
- `./gradlew.bat :app:clean :app:make` OK -> artefact `app/build/app.cs3` recopie vers `gramflix-all.cs3` + `gramflix-1jour1film.cs3` (95 654 o, plugin 21).
...
- Verifier sur GitHub que la release v1.4.16 et les assets `.cs3` restent accessibles (retenter upload si le token expire).
