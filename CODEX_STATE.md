...
- Filet de sécurité IMDB : fallback recherche/accueil via `home.json` + suggestion API, lecture directe `vidsrc`.
- Empaquetage `gramflix-all.cs3` via `gradlew.bat :app:make` (version 5, taille 48052 octets).
...
- Publier le tag `v1.4.0` + mise à jour de la release GitHub (remplacer l'asset `gramflix-all.cs3`).
